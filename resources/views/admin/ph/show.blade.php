...
$moduleSingularName = $ph_settings['identity_called'];
$modulePluralName = $ph_settings['identity_called'];

$pageTitle = $modulePluralName . ' - ' . $ph_item->transaction_code . ' Detail';
$moduleTitle = $modulePluralName;
$module = 'ph';

$itemStatus = $ph_item->status;
...
$breadCrumbArray = array(
    'Home' => url('admin'),
    $ph_settings['identity_called'] . ' List' => url('admin/ph'),
    $modulePluralName => '',
);
?>
...
<div class="box box-block bg-white">
    ...
    <p class="text-xs-right">{{ $ph_settings['identity_called'] }} Created at : {{ $ph_item->created_at }}</p>
    ...
    <div class="table-responsive">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                <tr>
                    <td>{{ $ph_item->transaction_code }}</td>
                    <td class="text-xs-right">{{ $web_settings['currency_type'] }} {{ $ph_item->amount }}</td>
                    <td class="text-xs-right">{{ $web_settings['currency_type'] }} {{ $ph_item->pending_amount }}</td>
                </tr>
            </tbody>
        </table>

        <?php $user_information = json_decode($ph_item->user_information); ?>
            ...
            <tbody>
                <tr>
                    <td class = "col-md-4">
                        ...
                        @if ($ph_item->user->user_id > 0)
                        <h6><u>Referer</u></h6>
                        <p>
                            <b>Username:</b> {{ $ph_item->user->sponsor->username }}<br/>
                            <b>Name:</b> {{ $ph_item->user->sponsor->name }}<br/>
                            <b>Email:</b> {{ $ph_item->user->sponsor->email }}<br/>
                            <b>Contact Number:</b> {{ $ph_item->user->sponsor->phone }}<br/>
                        </p>
                        @endif
                    </td>
                    <td class="col-md-4">
                        ...
                        <tbody>
                            <tr>
                                <td class = "col-md-4">
                                    ...
                                    @if ($pair_item->GH->user_id > 0)
                                    @if ($pair_item->GH->user->user_id > 0)
                                    <h6><u>Referer</u></h6>
                                    <p>
                                        <b>Username:</b> {{ $pair_item->GH->user->sponsor->username }}<br/>
                                        <b>Name:</b> {{ $pair_item->GH->user->sponsor->name }}<br/>
                                        <b>Email:</b> {{ $pair_item->GH->user->sponsor->email }}<br/>
                                        <b>Contact Number:</b> {{ $pair_item->GH->user->sponsor->phone }}<br/>
                                    </p>
                                    @endif
                                    @endif
                                </td>
