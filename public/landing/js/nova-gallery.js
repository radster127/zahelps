(function($) {
    ...
    $(document).on('add.cards change.cards', function(event) {
        if(typeof $.fn.masonry !== 'undefined') {
            $(event.target).outerFind('.nova-gallery').each(function() {
                var $msnr = $(this).find('.nova-gallery-row').masonry({
                    itemSelector: '.nova-gallery-item',
    ...
    function fitLightbox() {
        var $lightbox = $('.nova-gallery .modal');
        if(!$lightbox.length) {
            return;
        }
